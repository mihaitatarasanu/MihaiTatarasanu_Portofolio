...
/*We can see that there is one row left in the "Status" column with a blank value.*/
SELECT * 
FROM world_life_expectancy
WHERE Status = ''
;
...
It has a positive correlation with GDP. The higher the GDP, the higher life expectancy is.*/
...
It's easier to keep a higher average life expectancy for developed countries since there are fewer of them than developing countries.*/
