...
/*Let's bring all the records from the 'us_project.us_household_income_statistics' table whose 'id' does not match any values in the 'id' column from the 'us_project.us_household_income' table. Now the question is: what do we do with all of this data?*/
SELECT * 
...
/*Since "Municipality" has the highest value, let's investigate further by counting how many types of each kind are involved in the average calculation. We can see that there is only one "Municipality".*/
...
;
/*Let's add the average median income. We can observe that the value 300,000 repeats often. It might be a cap placed there, so it should be investigated by looking at the source data in order to understand what it is about.*/
